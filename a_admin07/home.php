...
'#546de5','#ea8685','#574b90','#ffd32a','#0fbcf9','#05c46b','#8919FE','#ff3f34');?>
            ...
            <?php
                ...
                $sql_course_year_count = $mysqli -> query($sql_num_year);
                
                
                
                while($row = $sql_course_year_count-> fetch_assoc()){
                    
                            
                    

                    $sql__city_count = "SELECT `city`,count(`id`) as 'count_city' FROM `course` WHERE `y1` = '{$row['y1']}' GROUP by `city`";
                    $sql__city_count = $mysqli -> query($sql__city_count);
                    
                    $nn = 0;
                    $labels_Chart ='';
                    $backgroundColor_Chart ='';
                    $data_Chart ='';
                    while($row_city_count = $sql__city_count-> fetch_assoc()){
                        
                        //echo $row_city_count['city'];
                    $sql__city_count_name = "SELECT `name` FROM `cities` WHERE `id` = '{$row_city_count['city']}'";
                    $sql__city_count_name = $mysqli -> query($sql__city_count_name);
                    $row_city_count_name = $sql__city_count_name-> fetch_assoc();
                            ...
                            <div class="inner-sbg">
                                ...
                                <div class="d-flex justify-content-between pt-5">
                                    <div class="">
                                        ...
                                        <span><i class="fa fa-chevron-up mr-2"></i><?php echo mysqli_num_rows($sql_course_up); ?></span>
                                    </div>
                                    <div class="">
                                        <h4>Previous Courses</h4>
                                        <span><i class="fa fa-chevron-down mr-2"></i><?php echo mysqli_num_rows($sql_num_old); ?></span>
                                    </div>
                ...
                <div class="col-md-4">
                    <div class="">
                            ...
                            $sql_DATEDIFF = $mysqli -> query($sql_DATEDIFF);
                            $nn = 0;
                            ?><div class="card-home mb-4"><?php
                            while($row_DATEDIFF = $sql_DATEDIFF-> fetch_assoc()){
