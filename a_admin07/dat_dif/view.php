...
$sql_course_ar_c1 = " SELECT *,count(`id`) as 'count' from ( SELECT `id`,`c_id`,DATEDIFF(CONCAT(`y2`,'-',`m2`,'-',`d2`),CONCAT(`y1`,'-',`m1`,'-',`d1`))+1 as 'DATEDIFF' FROM `course`
where DATEDIFF(CONCAT(`y2`,'-',`m2`,'-',`d2`),CONCAT(`y1`,'-',`m1`,'-',`d1`))+1 = '{$dat_dif}'
 ORDER BY `course`.`y1` ASC) as t group by `t`.`c_id`";
