...
LIMIT 0 , 1")-> fetch_assoc();
	...
	if (($result_course && (mysqli_num_rows($result_course) > 0)) or $_POST['c_id'] =='') {
		...
		$mysqli -> query($aa);
		$id = $mysqli->insert_id;
		topnotification('The course Brochure has been added');
		admin_history('Add Course',$admin_history_add,$row_history['id']);
		$link = "?page=course_main-edit&id={$id}{$topnotificationAddToLinke}&activeTab=Brochure";
		replace_aa();
	}
	...
	$mysqli -> query($aa);
	topnotification('The course Brochure has been Updated');
