...
function select_name_city($name,$view_name){
	global $conn,$city_nn,$addtocitytablename;
	$city_nn++;
	?>
<tr style="<?php if($city_nn !=1){ echo 'display:none'; }else{ echo 'display:compact'; }; ?>"
    ...
    <td>
				<select class="form-control d-inline-block" name="<?php echo $name; ?>" style="width:150px">
								...
								<?php 
				echo $sql_cities = "SELECT * FROM `cities{$addtocitytablename}`";
				...
				?>
        </select> <?php if($city_nn == 1){ ?><a href="#"  class="btn btn-danger btn-sm  py-1  float-right"  id="city_a_Remove"
...
function select_name_city_Remove($name,$view_name){
	global $conn,$city_Remove_nn,$addtocitytablename;
	$city_Remove_nn++;
	?>
<tr style="display:none;" class="alert-danger" id="city_Remove_<?php echo $city_Remove_nn;?>">
    ...
    <td><select name="Remove_<?php echo $name; ?>" style="width:150px">
            <option value="">&nbsp;</option>
            <?php 
		$sql_cities = "SELECT * FROM `cities{$addtocitytablename}`";
		$result_cities = mysql_query($sql_cities, $conn);
		if ($result_cities && (mysql_num_rows($result_cities) > 0)) {
			while ($row_cities = mysql_fetch_assoc($result_cities)){
				?>
...
function select_name_c_id($name,$view_name){
	global $conn,$c_id_nn,$addtoc_idtablename;
	$c_id_nn++;
	?>
...
function select_name_c_id_Remove($name,$view_name){
	global $conn,$c_id_Remove_nn,$addtoc_idtablename;
	$c_id_Remove_nn++;
	?>
...
//categories
		...
		<tr style="<?php if($category_nn !=1){ echo 'display:none'; }else{ echo 'display:compact'; }; ?>"
            ...
            id="category_a_Remove" onclick="document.getElementById('category_Remove_1').style.display='';document.getElementById('category_a_Remove').style.display='none';">-
            Expect Category</a><?php }; ?>

        <?php if($category_nn <= 11){ ?><a href="#"  class="btn btn-success btn-sm  mr-2  py-1  float-right"  id="category_a<?php echo $category_nn;?>"
...
function select_name_category_Remove($name,$view_name){
	global $conn,$category_Remove_nn;
	$category_Remove_nn++;
	?>
...
<tr style="<?php if($Month_nn !=1){ echo 'display:none'; }else{ echo 'display:compact'; }; ?>"
    ...
    <td>
			...
			<?php if($Month_nn == 1){ ?><a href="#"   class="btn btn-danger btn-sm   py-1  float-right"  
            id="Month_a_Remove"
            onclick="document.getElementById('Month_Remove_1').style.display='';document.getElementById('Month_a_Remove').style.display='none';">-
            Expect Months</a><?php }; ?>
        <?php if($Month_nn <= 11){ ?><a href="#"   class="btn btn-success btn-sm mr-2  py-1  float-right"   id="Month_a<?php echo $Month_nn;?>"
...
<tr style="display:none;"  class="alert-danger" id="Month_Remove_<?php echo $Month_Remove_nn;?>">
    ...
    <td><input name="<?php echo $name; ?>" style="width:150px">
        <?php if($Month_Remove_nn <= 11){ ?><a href="#" id="Month_Remove_a<?php echo $Month_Remove_nn;?>"  class="btn btn-danger btn-sm  py-1  float-center" 
...
function select_name_Year($name,$view_name){
	global $conn,$Year_nn,$Year_defult;
	$Year_nn++;
	?>
<tr style="<?php if($Year_nn !=1){ echo 'display:none'; }else{ echo 'display:compact'; }; ?>"
    id="Year_<?php echo $Year_nn;?>">
    <th scope="col"><?php echo $view_name; ?></th>
    <td><input class="form-control d-inline-block" name="<?php echo $name; ?>" style="width:150px" value="<?php if($Year_nn ==1){ echo $Year_defult;}?>">
		<?php if($Year_nn == 1){ ?><a href="#"   class="btn btn-danger btn-sm    py-1  float-right"   id="Year_a_Remove"
            ...
            Except Year </a><?php }; ?>
        <?php if($Year_nn <= 11){ ?><a href="#"   class="btn btn-success mr-2 btn-sm   py-1  float-right"   id="Year_a<?php echo $Year_nn;?>"
...
function select_name_Year_Remove($name,$view_name){
	global $conn,$Year_Remove_nn;
	$Year_Remove_nn++;
	?>
<tr style="display:none;"  class="alert-danger" id="Year_Remove_<?php echo $Year_Remove_nn;?>">
    <th scope="col"><?php echo $view_name; ?></th>
    <td><input name="<?php echo $name; ?>" style="width:150px">
        <?php if($Year_Remove_nn <= 11){ ?><a href="#" id="Year_Remove_a<?php echo $Year_Remove_nn;?>"  class="btn btn-danger btn-sm  py-1  float-center" 
						...
						<div class="table-responsive">
							<table class="table">
								...
								<tr>
										...
										<td><input class="form-control col-4" type="text" name="view_one_of" value="1" /></td>
								</tr>
								<tr>
								...
								<tr>
										...
										<td><input class="form-control col-4"  type="text" name="format" /></td>
								</tr>

								<tr>
										<th scope="col">Max Records</th>
										<td><input class="form-control col-4"  type="text" name="Max_Records" value="500" /></td>
								</tr>

								<tr>
										<th scope="col">&nbsp;</th>
										<td><input type="submit" name="button" id="button" class="btn col-5 btn-primary float-right" /></td>
								</tr>
							</table>
						</div>
					</div>
