...
if(isset($_GET["c_id"])){ $c_id = $_GET["c_id"]; }else{ 
	$sql_cid = "SELECT `c_id`,`city` FROM `course` WHERE 1 AND `id` LIKE '" . $id . "' ";
	$result_cid = $mysqli -> query($sql_cid);
	if ($result_cid && (mysqli_num_rows($result_cid) > 0)) {
		$row_cid = $result_cid-> fetch_assoc();
        $c_id =$row_cid['c_id'];
        $thisEventCity = $row_cid['city'];
        $sql_monday = "SELECT `monday` FROM `cities` where `id` = '{$thisEventCity}'";
        $result_monday = $mysqli -> query($sql_monday);
        if ($result_monday && (mysqli_num_rows($result_monday) > 0)) {
            $row_monday = $result_monday-> fetch_assoc();
            $thisEventMonday = $row_monday['monday'];
            if($thisEventMonday == 1){
                ?><script>
...
if ($resultthecourse_c && (mysqli_num_rows($resultthecourse_c) > 0) and $ErrInWeek == 0) {
...
<form method="post" action="?page=course-replace" name="insertForm" id="insertForm">
    ...
    <div class="main-content-inner">
        <div class="row">
            ...
            <div class="col-lg-3 mt-5 weeksSunday">
            ...
            <div class="col-lg-3 mt-5 weeksMonday ">
...
<script>
$(document).ready(function(){
  $(".showWeeksMonday").click(function(){
    $(".weeksSunday").hide();
    $(".weeksMonday").show();
  });
  $(".showWeeksSunday").click(function(){
    $(".weeksSunday").show();
    $(".weeksMonday").hide();
  });
});
                                ...
                                <table id="table-s" class="table text-center table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        <tr onclick="
                                              ...
                                              " class="<?php if($row_select['monday'] == '1'){ ?>showWeeksMonday<?php }else{ ?>showWeeksSunday<?php } ?>">
                                            <td <?php
