...
//             }
//             $pagination = '<nav><ul class="pagination justify-content-center pt-3">';
...
class CallAPIv3 {
    ...
    public function __construct($scope, $httpMethod = self::HTTP_GET, $lang = 'ar') {
        global $systemUrl;
        $host = $systemUrl;
    ...
    function pagination(){
        ...
        if($this->httpCode == 200 and isset($this->result->{'totalPages'}) and $this->result->{'totalPages'} > 1){
            ...
            $pagination = '<nav class=""><ul class="pagination justify-content-center pt-3">';
            // if($page_no > 1 and $startLast ==1){
            //     $pagination .='<li class="page-item"><a href="'.$firstPageLink.'" class="page-link">'.$firstTitle.'</a></li>';
            // };
            // if($page_no > 1 and $previousNext ==1){
            ...
            foreach ($breadCrumbsArray as &$value) {
                ...
                if($nn == $countBC){
                  ...
                  <meta itemprop="position" content="'.$nn.'" />
                    ...
                    $breadCrumbs .='<li class="breadcrumb-item" itemprop="itemListElement" itemscope
                    itemtype="https://schema.org/ListItem">
                  <a itemprop="item" title="'.($value->title == '' ? 'الصفحة الرئيسية' : $value->title).'" href="'.$systemUrl.$value->link.'" >
