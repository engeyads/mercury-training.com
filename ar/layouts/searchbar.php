            ...
            <div class="row" style="">
                ...
                <div class="col-xs-12 col-sm-3 col-md-2 col-lg-2 col-xl-2 mb-2">
                    <div class="form-group">
                        ...
                        ?>
                        <input class="form-control form-control-sm" type="text" id="keyword" name="keyword" placeholder="بحث..." aria-label="default input example" value="<?php echo $addedKeyword != null ? $addedKeyword : ''; ?>">
                    </div>
                </div>
                <div class="col-xs-12 col-sm-3 col-md-2 col-lg-2 col-xl-2 mb-2">
                    <div class="form-group">
                            ...
                            <?php
                                ...
                                if (isset($categoriesArray->result->categories) && (count($categoriesArray->result->categories) > 0)) {
                                    ...
                                    foreach ($categories as &$value) {
                                            ?>
                                            <option value="<?php echo $value->id; ?>" <?php echo ($selectedCategory != null && $selectedCategory == $value->id) ? 'selected' : '' ?>>
                                                <?php echo $value->name; ?>
                                            </option>
                                            <?php
                                    }
                ...
                <div class="col-xs-12 col-sm-1 col-md-1 col-lg-1 col-xl-1 mb-2">
                    <div class="form-group">
                        <select class="form-select form-select-sm" aria-label="Default select example" name="numberOfWeeks" id="numberOfWeeks">
                            <option selected value="">المدة</option>
                            <option value="1" <?php if(isset($_GET['numberOfWeeks']) and $_GET['numberOfWeeks'] == 1){ ?>selected<?php }?>>اسبوع</option>
                            <option value="2" <?php if(isset($_GET['numberOfWeeks']) and $_GET['numberOfWeeks'] == 2){ ?>selected<?php }?>>اسبوعان</option>
                        </select>
                    </div>
                </div>
                <div class="col-xs-12 col-sm-2 col-md-2 col-lg-2 col-xl-2 mb-2">
                    <div class="form-group">
                        <select class="form-select form-select-sm" aria-label="Default select example" name="upcoming" id="upcoming">
                            <option value="false" <?php if(isset($_GET['upcoming']) and $_GET['upcoming'] == 'false'){ ?>selected<?php }?>>جميع الدورات</option>
                            <option value="true" <?php if(isset($_GET['upcoming']) and $_GET['upcoming'] == 'true'){ ?>selected<?php }?>>المؤكدة فقط</option>
                        </select>
                    </div>
                </div>
                <div class="col-xs-12 col-sm-12 col-md-2 col-lg-2 col-xl-2 mb-2 d-flex justify-content-center justify-content-md-end">
                    <button type="submit" class="btn btn-search p-3 p-md-1 btn-sm w-100">بحث</button>
                </div>
            </div>
        </form>
    </div>
